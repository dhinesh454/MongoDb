...
async function saveExpenses(event)
{
    ...
    try {
        ...
        const res=await axios.post('http://localhost:3000/expense/addexpenses',details,{headers:{"Authorisation":token}});
...
try {
    ...
    const page =1
    const res = await axios.get(`http://localhost:3000/expense/getexpenses?page=${page}&pageSize=${pageSize}`,{headers:{"Authorisation":token}});
...
async function deleteExpense(id)
{
    try {
        const token=localStorage.getItem('token');
        console.log(id);
        const res=await axios.delete(`http://localhost:3000/expense/delete-expense/${id}`,{headers:{"Authorisation":token}});
...
document.getElementById('rzp-button').onclick=async (e)=>{
    const token = localStorage.getItem('token');
    const res=await axios.get('http://localhost:3000/purchase/premiummembership',{headers:{"Authorisation":token}});
    console.log(res);
...
function showleaderBoard()
{
    ...
    const token = localStorage.getItem('token');
    const userLeaderboardArray=await axios.get('http://localhost:3000/premium/showLeaderBoard',{headers:{"Authorisation":token}});
...
async function download(){
    try {
        ...
        const response = await axios.get('http://localhost:3000/user/download',{headers:{"Authorisation":token}});
        console.log(response);
...
async function pageSize(val){
    try {
        ...
        const page=1
        const res = await axios.get(`http://localhost:3000/expense/getexpenses?page=${page}&pageSize=${val}`,{headers:{"Authorisation":token}});
      console.log('success');
        console.log(res);
...
function showOnScreen(user)   
{
    ...
    <button class="btn btn-danger" onclick="deleteExpense('${user._id}')">DeleteExpense</button></li>`
