...
const updateTransactionStatus=async(req,res,next)=>{
    try {
        ...
        const promise1=order.updateOne({paymentid:payment_id,status:'SUCCESSFULL',userId:req.user.id});

        const promise2=User.updateOne({_id:req.user._id},{ispremiumuser:true});

        await Promise.all([promise1,promise2]);

        return res.status(202).json({success:true,message:"Transcation successfull",token:generateAccessToken(req.user._id,req.user.name,true)})
