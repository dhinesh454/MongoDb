...
const forgotPassword=async(req,res,next)=>{
    try {
        ...
        console.log('>>>into if user forgot password ---apikey',process.env.SENDINBLUE_API_KEY);
            ...
            console.log(user.id);
            await Forgotpassword.create({action:true,userId:user._id});
 ...
 const resetpassword = async(req, res) => {
    ...
    const forgetpassword=await Forgotpassword.findById(id);
    console.log(forgetpassword);
        if(forgetpassword){
            await Forgotpassword.updateOne({id:forgetpassword.id},{ active: false});
            res.status(200).send(`<html>
                                    <script>
                                        async function formsubmitted(e){
                                            e.preventDefault();
                                            const res=await axios.get('http://localhost:3000/password/updatepassword/${id}');
